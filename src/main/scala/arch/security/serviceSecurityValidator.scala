...
class ServiceSecurityValidator[A: ValidatorSource](conf: A, client: Client[IO]) extends SecurityValidator[IO]{
  ...
  given Semigroup[String] = Semigroup.instance[String]((a, b) => a)
...
//      _ <- IO.println(body)
      _ <- {
        ...
        val kmap: Map[String, JWK] = keys.map { k =>
          k.getKeyID -> k
        }.toMap
        jwkSet = Some(kmap)
        IO(())
      ...
      case Some(kmap) =>
        ...
        if (sset.contains(kid)) {
          ...
          if (isValid) {
            ...
            generalChecks.combine(Validated.cond(hasRole, id, List((JWTErrors.RolesInvalid, s"JWT roles invalid: roles '${rolesToCheck.mkString("{", ", ", "}")}' not in '${roles.mkString("{", ", ", "}")}'"))))
          } else generalChecks
        }else{
          Validated.invalid(List((JWTErrors.KidNotPresentInJWKS, s"Key id '$kid' not match anyone of the present in the JWKS"))) // "left" value
        }
      case None =>
        Validated.invalid(List((JWTErrors.NotJWKS, "Not JWKS"))) // "left" value
    }
    valid.toEither
  }
